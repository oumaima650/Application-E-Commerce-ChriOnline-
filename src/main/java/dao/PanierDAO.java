...
public class PanierDAO {
     ...
     * C'est beaucoup plus performant car on n'a pas besoin de charger toutes les
     * lignes en Java.
     */
    ...
    public List<LignePanier> getLignesByPanierId(int idPanier) {
        ...
        try (Connection conn = ConnexionBDD.getConnection();
            ...
            while (rs.next()) {
                ...
                BigDecimal sousTotal = prix != null ? prix.multiply(new BigDecimal(rs.getInt("quantite")))
                        : BigDecimal.ZERO;
    ...
    public List<LignePanier> getLignesParSkus(int idPanier, List<String> skus) {
        ...
        StringBuilder query = new StringBuilder(
                "SELECT lp.*, s.prix, s.image FROM LignePanier lp JOIN SKU s ON lp.SKU = s.SKU WHERE lp.idPanier = ? AND lp.SKU IN (");
