...
import client.ProduitDetailService;
...
public class ProductDetailController implements Initializable {
    ...
    @FXML private HBox thumbnailBar;
    ...
    private void loadProductComplet() {
        System.out.println("[ProductDetailController] loadProductComplet called with selectedProductId: " + selectedProductId);
    ...
    private void displayProductInfo() {
        ...
        if (produitData.containsKey("stockTotal")) {
            ...
            stockLabel.setStyle("-fx-text-fill: " + (stockTotal > 0 ? "#10B981" : "#EF4444") + "; -fx-font-weight: bold;");
        }
    ...
    private void setupImageGallery() {
        if (thumbnailBar == null || allSkus.isEmpty()) return;
        ...
        for (int i = 0; i < allSkus.size(); i++) {
            ...
            if (imageUrl != null && !imageUrl.isBlank()) {
                ...
                thumbContainer.setStyle("-fx-background-color: white; -fx-border-color: #DDD; -fx-border-radius: 8px; -fx-background-radius: 8px; -fx-cursor: hand;");
                ...
                if (i == currentImageIndex) {
                    thumbContainer.setStyle("-fx-background-color: " + CORAIL + "; -fx-border-color: " + CORAIL + "; -fx-border-radius: 8px; -fx-background-radius: 8px; -fx-cursor: hand;");
                }
    ...
    private void selectImage(int index) {
        if (index < 0 || index >= allSkus.size()) return;
    ...
    private void updateThumbnailSelection() {
        if (thumbnailBar == null) return;
        
        for (int i = 0; i < thumbnailBar.getChildren().size(); i++) {
            StackPane container = (StackPane) thumbnailBar.getChildren().get(i);
            if (i == currentImageIndex) {
                container.setStyle("-fx-background-color: " + CORAIL + "; -fx-border-color: " + CORAIL + "; -fx-border-radius: 8px; -fx-background-radius: 8px; -fx-cursor: hand;");
            } else {
                container.setStyle("-fx-background-color: white; -fx-border-color: #DDD; -fx-border-radius: 8px; -fx-background-radius: 8px; -fx-cursor: hand;");
            }
        }
    }

    private void updateVariantesFromSku(Map<String, Object> sku) {
        // Cette méthode met à jour les sélecteurs de variantes pour correspondre au SKU sélectionné via la galerie
        // Pour l'instant, nous allons simplement mettre à jour le SKU actuel et l'affichage
        currentSku = sku;
        updateSkuDisplay();
        
        // TODO: Implémenter la logique pour extraire les valeurs de variantes du SKU et mettre à jour les ComboBox
        // Ceci nécessiterait une requête supplémentaire ou une structure de données différente
    }

    private void setupVariantSelectors() {
        if (variantsContainer == null || uiData == null) return;
        ...
        for (Map.Entry<String, List<String>> entry : variantes.entrySet()) {
            ...
            if (nomVariante.equalsIgnoreCase("Couleur")) {
                ...
                for (String val : valeurs) {
                    ...
                    if (imgUrl != null) {
                        try {
                            ...
                            btn.setText(val);
                            btn.setStyle("-fx-background-color: white; -fx-border-color: #DDD; -fx-border-radius: 20px; -fx-background-radius: 20px; -fx-padding: 5 15; -fx-cursor: hand;");
                        }
                    } else {
                        btn.setText(val);
                        btn.setStyle("-fx-background-color: white; -fx-border-color: #DDD; -fx-border-radius: 20px; -fx-background-radius: 20px; -fx-padding: 5 15; -fx-cursor: hand;");
                    ...
                    if (currentSelections != null && val.equals(currentSelections.get(nomVariante))) {
                        ...
                        if (imgUrl == null) {
                            btn.setStyle("-fx-background-color: " + CORAIL + "; -fx-text-fill: white; -fx-border-color: " + CORAIL + "; -fx-border-radius: 20px; -fx-background-radius: 20px; -fx-padding: 5 15; -fx-cursor: hand;");
                        } else {
                            btn.setStyle("-fx-background-color: transparent; -fx-border-color: " + CORAIL + "; -fx-border-width: 2px; -fx-border-radius: 50%; -fx-cursor: hand; -fx-padding: 2;");
                        }
                    }
                        ...
                        if (btn.isSelected()) {
                            for (Toggle t : colorGroup.getToggles()) {
                                ...
                                if (tb.getGraphic() != null) {
                                    tb.setStyle("-fx-background-color: transparent; -fx-border-color: #DDD; -fx-border-width: 1px; -fx-border-radius: 50%; -fx-cursor: hand; -fx-padding: 2;");
                                } else {
                                    tb.setStyle("-fx-background-color: white; -fx-text-fill: black; -fx-border-color: #DDD; -fx-border-radius: 20px; -fx-background-radius: 20px; -fx-padding: 5 15; -fx-cursor: hand;");
                                }
                            }
                ...
                combo.setStyle("-fx-background-color: white; -fx-border-color: #DDD; -fx-border-radius: 8px; -fx-background-radius: 8px; -fx-padding: 5 10;");
                ...
                if (currentSelections != null && currentSelections.containsKey(nomVariante)) {
                    ...
                    if (currentSelections == null) currentSelections = new HashMap<>();
                    currentSelections.put(nomVariante, valeurs.get(0));
                }
    ...
    private Map<String, Object> trouverSkuParVariantes(Map<String, String> selections) {
        if (uiData == null || selections == null) return null;
        @SuppressWarnings("unchecked")
    ...
    private void updateSkuDisplay() {
        ...
        if (currentSku.containsKey("quantite")) {
            ...
            // Désactiver le bouton si pas de stock
            addToCartButton.setDisable(stock == 0);
            if (stock == 0) {
                ...
                addToCartButton.setStyle("-fx-background-color: " + CORAIL + "; -fx-text-fill: white;");
            }
        }
    }

    private void loadProductImage() {
        if (productImageView == null || currentSku == null) return;
    ...
    @FXML private void decrementQty() { 
        if (quantity > 1) { 
            quantity--; 
            quantityLabel.setText(String.valueOf(quantity)); 
        } 
    ...
    @FXML private void handleAddToCart() {
        ...
        Task<Reponse> task = new Task<>() {
            @Override protected Reponse call() {
                ...
                return sendRequest(new Requete(RequestType.ADD_TO_CART, params, SessionManager.getInstance().getSession().getToken()));
            }
        };
        task.setOnSucceeded(e -> {
    ...
    private void updateCartBadge() {
        Task<Reponse> task = new Task<>() {
            @Override protected Reponse call() {
                ...
                return sendRequest(new Requete(RequestType.GET_CART, params, SessionManager.getInstance().getSession().getToken()));
            }
        };
        task.setOnSucceeded(e -> {
            Reponse rep = task.getValue();
    ...
    private void showToast(String msg) {
        ...
        if (mainContent != null && mainContent.getScene() != null) {
            ...
            toast.setStyle("-fx-background-color: " + BLEU_NUIT + "; -fx-text-fill: white; -fx-padding: 10 20; -fx-background-radius: 50px;");
            ...
            new Timeline(new KeyFrame(javafx.util.Duration.seconds(3), ev -> root.getChildren().remove(stackPane))).play();
        }
    }
}
